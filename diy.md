...
{% comment %} Collect all unique tags from diy {% endcomment %}
{% assign all_tags = "" %}
{% for post in diy %}
...
<ul class="post-list">
  ...
  <li class="post-item" data-tags="{% for tag in post.tags %}{{ tag }} {% endfor %}">
    <div class="post-meta">
      ...
      {% for tag in post.tags %}
        <span class="post-tag" style="background: var(--tag-tech); color: var(--tag-tech-text);">{{ tag | replace: '_', '' }}</span>
      {% endfor %}
    </div>
...
<div class="empty-state" id="no-results" style="display: none;">
  <p>No DIY projects found for this tag.</p>
</div>
